			                                ...
			                                uint16_t* const BytesProcessed)
{
	...
	for(i=0;i<Length;i++)
	{
		...
		if (endpointselected!=ENDPOINT_CONTROLEP)
			while(usb_data_buffer_OUT_size == 0) Delay_MS(1); /* Current Fix for LPC17xx, havent checked for others */
		#endif
		((uint8_t*)Buffer)[i] = Endpoint_Read_8();
	}
	return ENDPOINT_RWSTREAM_NoError;
...
uint8_t Endpoint_Read_Control_Stream_LE(void* const Buffer,
			                                        uint16_t Length)
{
	while(!Endpoint_IsOUTReceived()) Delay_MS(1);			//FIXME: this safe checking is fine for LPC18xx
