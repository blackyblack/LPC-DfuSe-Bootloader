	...
	/* Private Interface - For use in library only: */
		...
		/* Inline Functions: */
			...
			static inline uint16_t Endpoint_BytesInEndpoint(void)
			{
				...
				else
				{
					//return (dmaDescriptor[ endpointhandle[endpointselected] ].PresentCount);
                                  ///HACK: a copy of Endpoint_GetEndpointDirection.
                                  ///      We return total written bytes if endpoint has IN direction.
                                  ///      Original code does not have this line and always returns
                                  ///      usb_data_buffer_OUT_size
                                  if (endpointhandle[endpointselected]%2)
                                  {
                                          return usb_data_buffer_IN_index;
                                  }
                                  else
                                  {
			...
			static inline bool Endpoint_IsINReady(void)
			{
				...
				{
					uint8_t SelEP_Data;
					if (dmaDescriptor[ endpointhandle[endpointselected] ].Retired){
