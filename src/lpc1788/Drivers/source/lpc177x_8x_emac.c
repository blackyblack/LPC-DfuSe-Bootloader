...
static volatile bool phy_ok;

#pragma location="EMAC_DMA_RAM3"
#pragma data_alignment=4
__no_init unsigned char emac_rx_desc[EMAC_NUM_RX_FRAG*8];

#pragma location="EMAC_DMA_RAM3"
#pragma data_alignment=4
__no_init unsigned char emac_rx_stat[EMAC_NUM_RX_FRAG*8];

#pragma location="EMAC_DMA_RAM3"
#pragma data_alignment=4
__no_init unsigned char emac_tx_desc[EMAC_NUM_TX_FRAG*8];

#pragma location="EMAC_DMA_RAM3"
#pragma data_alignment=4
__no_init unsigned char emac_tx_stat[EMAC_NUM_TX_FRAG*4];

//Should be 8 byte aligned according to DS (Page 253)
#pragma location="EMAC_DMA_RAM3"
#pragma data_alignment=8
__no_init unsigned char emac_rx_buf[EMAC_NUM_RX_FRAG*EMAC_ETH_MAX_FLEN];

#pragma location="EMAC_DMA_RAM3"
#pragma data_alignment=4
__no_init unsigned char emac_tx_buf[EMAC_NUM_TX_FRAG*EMAC_ETH_MAX_FLEN];

/* EMAC variables located in 16K Ethernet SRAM */
#define RX_DESC_BASE        (uint32_t)emac_rx_desc
#define RX_STAT_BASE        (uint32_t)emac_rx_stat
#define TX_DESC_BASE        (uint32_t)emac_tx_desc
#define TX_STAT_BASE        (uint32_t)emac_tx_stat
#define RX_BUF_BASE         (uint32_t)emac_rx_buf
#define TX_BUF_BASE         (uint32_t)emac_tx_buf
...
void write_PHY (int PhyReg, int Value)
{
	...
	LPC_EMAC->MADR = EMAC_PHY_DEF_ADR | PhyReg;
	LPC_EMAC->MWTD = Value;
...
void setEmacAddr(uint8_t abStationAddr[])
{
	...
	LPC_EMAC->SA0 = ((uint32_t)abStationAddr[0] << 8) | (uint32_t)abStationAddr[1];
	LPC_EMAC->SA1 = ((uint32_t)abStationAddr[2] << 8) | (uint32_t)abStationAddr[3];
	LPC_EMAC->SA2 = ((uint32_t)abStationAddr[4] << 8) | (uint32_t)abStationAddr[5];
}
...
int32_t EMAC_Init(EMAC_CFG_Type *EMAC_ConfigStruct)
{
	...
	LPC_EMAC->MCFG = EMAC_MCFG_CLK_SEL(0x0B) | EMAC_MCFG_RES_MII;

	// release reset
	LPC_EMAC->MCFG &= (uint32_t)~(EMAC_MCFG_RES_MII);
	...
	LPC_EMAC->RxFilterCtrl = EMAC_RFC_MCAST_EN | EMAC_RFC_BCAST_EN | EMAC_RFC_PERFECT_EN | EMAC_RFC_UCAST_EN;
...
//ѕопытка установить скорость передачи.
//≈сли EMAC_Init не был выполнен, то всегда будет возвращать false.
bool EMAC_Autonegotiation()
{
  //если PHY не инициализирован, то не идЄм дальше
  if(!phy_ok)
    return false;
  
  //начинаем процедуру определени€ сети
  int res = EMAC_SetPHYMode(EMAC_MODE_AUTO);
...
{
	LPC_EMAC->Command &= (uint32_t)~0x02;
	return;
}
...
{
	LPC_EMAC->Command &= (uint32_t)~0x01;
	LPC_EMAC->MAC1 &= (uint32_t)~0x01;
	return;
}
...
int32_t EMAC_SetPHYMode(uint32_t ulPHYMode)
{
	...
	if (((id1 << 16) | (id2 & 0xFFF0)) == EMAC_PHY_ID)
	{
...
int32_t EMAC_UpdatePHYStatus(void)
{
	...
	regv = read_PHY(PHY_REG_SPEED);
        if(regv == (-1))
          return (-1);

        regv = (regv >> 2) & 0x07;  //-- Get bits 4:2
        
        if(regv == 2)       //-- [010] = 100BASE-TX half-duplex.
         {
           LPC_EMAC->MAC2       &=    (uint32_t)~0x1;       //HALF Duplex Mode
           LPC_EMAC->Command    &=    (uint32_t)~(1<<10);   //Set NOT Full Duplex Mode
           LPC_EMAC->IPGT       =     0x0012;     //-- IPGT_HALF_DUP
           LPC_EMAC->SUPP       |=    (1<<8);     //-- RMII - for 100Mb
           
           debug(DEBUG_LEVEL_INFO, "NETIF: 100BASE-TX half-duplex\r\n");
         }
         else if(regv == 5)  //-- [101] = 10BASE-T full-duplex.
         {
           LPC_EMAC->MAC2       |=    0x1;        //-- MAC2_FULL_DUP
           LPC_EMAC->Command    |=    (1<<10);    //-- CR_FULL_DUP
           LPC_EMAC->IPGT       =     0x0015;     //-- IPGT_FULL_DUP
           LPC_EMAC->SUPP       &=    (uint32_t)~(1<<8);    //-- RMII - for 10Mb
           
           debug(DEBUG_LEVEL_INFO, "NETIF: 10BASE-T full-duplex\r\n");
         }
         else if(regv == 6)  //-- [110] = 100BASE-TX full-duplex.
         {
           LPC_EMAC->MAC2       |=    0x1;        //-- MAC2_FULL_DUP
           LPC_EMAC->Command    |=    (1<<10);    //-- CR_FULL_DUP
           LPC_EMAC->IPGT       =     0x0015;     //-- IPGT_FULL_DUP
           ...
           debug(DEBUG_LEVEL_INFO, "NETIF: 100BASE-TX full-duplex\r\n");
         }
         else //-- [001] = 10BASE-T half-duplex or unknown(default)
         {
           ...
           debug(DEBUG_LEVEL_INFO, "NETIF: 10BASE-T half-duplex\r\n");
         }
...
void EMAC_WritePacketBuffer(EMAC_PACKETBUF_Type *pDataStruct)
{
	...
	idx = LPC_EMAC->TxProduceIndex;
        Mem_Copy((void *)TX_DESC_PACKET(idx), pDataStruct->pbDataBuf, pDataStruct->ulDataLen);
...
void EMAC_ReadPacketBuffer(EMAC_PACKETBUF_Type *pDataStruct)
 {
	...
	idx = LPC_EMAC->RxConsumeIndex;
        Mem_Copy(pDataStruct->pbDataBuf, (void *)RX_DESC_PACKET(idx), pDataStruct->ulDataLen);
...
{
        uint32_t idx;
	idx = LPC_EMAC->RxProduceIndex;
        
	if (LPC_EMAC->RxConsumeIndex != idx)
	{
		return TRUE;
	}
	else
...
//возвращает false, если была ошибка чтени€, неверна€ модель
static bool PHY_Init()
{
  int32_t id1, id2, rhy_id;

  /* Check if this is a DP83848C PHY. */
  id1 = read_PHY (EMAC_PHY_REG_IDR1);
  id2 = read_PHY (EMAC_PHY_REG_IDR2);
  rhy_id = (id1 << 16) | (id2 & 0xFFF0);

  debug(DEBUG_LEVEL_INFO, "PHY ID is %08X\r\n", rhy_id);
  
  if (rhy_id != EMAC_PHY_ID)
    return false;
