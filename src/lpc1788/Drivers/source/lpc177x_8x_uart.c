	...
	* 0 < mulFracDiv <= 15, 0 <= dividerAddFracDiv <= 15 */
	...
	for (m = 1 ; m <= 15 ;m++)
	{
		for (d = 0 ; d < m ; d++)
		{
			...
			{
				current_error = -(int32_t)current_error;
				tmp++;
			}
	...
	if (best_error < UART_ACCEPTED_BAUDRATE_ERROR)
	{
		if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1)
		{
			...
			/* Then reset DLAB bit */
			((LPC_UART1_TypeDef *)UARTx)->LCR &= ((uint32_t)~(int32_t)UART_LCR_DLAB_EN) & UART_LCR_BITMASK;
		...
		else
		{
			...
			/* Then reset DLAB bit */
			UARTx->LCR &= ((uint32_t)~((int32_t)UART_LCR_DLAB_EN)) & UART_LCR_BITMASK;
							...
							FunctionalState NewState)
{
	...
	if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1)
	{
		if (NewState == ENABLE)
		{
			...
			((LPC_UART1_TypeDef *)UARTx)->LCR &= (uint32_t)~(int32_t)UART_LCR_DLAB_EN;
	...
	else
	{
		if (NewState == ENABLE)
		{
			...
			UARTx->LCR &= (uint32_t)~(int32_t)UART_LCR_DLAB_EN;
...
void UART_TxCmd(LPC_UART_TypeDef *UARTx, FunctionalState NewState)
{
	...
	else
	{
		if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1)
		{
			((LPC_UART1_TypeDef *)UARTx)->TER &= ((uint32_t)~(int32_t)UART_TER_TXEN) & UART_TER_BITMASK;
		}
		else
		{
			UARTx->TER &= ((uint32_t)~(int32_t)UART_TER_TXEN) & UART_TER_BITMASK;
		}
	}
}
													...
													UART1_SignalState NewState)
{
	...
	{
		UARTx->MCR &= ((uint32_t)~(int32_t)tmp) & UART1_MCR_BITMASK;
	}
}
											...
											uint32_t size, uint8_t ParityStick)
{
	...
	if (ParityStick)
	{
		...
		tmp &= (uint32_t)~((int32_t)UART_LCR_PARITY_EVEN);
